...
;     (make-zero-crossingsNEW 
;      (stream-cdr input-stream) avpt))))
;
;(define zero-crossingsNEW 
;  (make-zero-crossingsNEW sense-data 0))
;
...
;  (make-zero-crossingsSOL sense-data 
;                       0
;                       0))
;
;(show-stream zero-crossings 11)
;(show-stream zero-crossingsNEW 11)
;(show-stream smoothed-zero-crossings 11)
;
;(define sense-data2 (make-my-stream (list 1 2 1.5 1 0.5 -0.1 -2 -3 -2 1.5 -2 3 4)))
;
;(define zero-crossings_old 
;  (make-zero-crossingsNEW sense-data2 0))
;
;(define smoothed-zero-crossings_new 
;  (make-zero-crossingsSOL sense-data2
;                       0
;                       0))
;
;(show-stream zero-crossings_old 11)
;(show-stream smoothed-zero-crossings_new 11)
...
(define sense-data3 (make-my-stream (list 1 2 1.5 1 0.5 -0.1 -2 -3 -2 -0.5 0.2 3 4)))

(define (make-zero-crossings75 
         input-stream last-value)
  (let ((avpt 
         (/ (+ (stream-car input-stream) 
               last-value) 
    ...
    (cons-stream 
     (sign-change-detector avpt last-value)
     (make-zero-crossings75 
      (stream-cdr input-stream) avpt))))

(define zero-crossings75 
  (make-zero-crossings75 sense-data3 0))
    ...
    (if (null? input-stream)
        ...
        (let ((avpt 
               (/ (+ (stream-car input-stream) 
                     last-value) 
                  2)))
          (cons-stream avpt (iter (stream-cdr input-stream) (stream-car input-stream))))))
  (iter stream 0))

;(define smoothed-sense-data
;  (smooth sense-data3))

;(show-stream smoothed-sense-data 11) ;ok works - take average of two successive items in stream

(define (make-zero-crossings76 input-stream)
...
(define zero-crossings76
  (make-zero-crossings76 sense-data3))

;(show-stream sense-data3 11)
;(show-stream zero-crossings75 11)
;(show-stream zero-crossings76 11)
...
(define (solve f y0 dt)
  (define y (integrall dy y0 dt))
  (define dy (stream-map f y))
  y)

(define (solvee f y0 dt)
  (define y (integrall (delay dy) y0 dt))
  (define dy (stream-map f y))
  y)

;(stream-ref 
; (solvee (lambda (y) y) 1 0.001) 1000)
;2.716924 - works in Racket!
   ...
   (if (stream-null? integrand)
       ...
       (integralo 
        ...
        (+ (* dt (stream-car integrand))
           initial-value)
        dt))))
;but the above needs to have delays added in to work as above - try it!
...
(define (solve-rev f y0 dt)
  (define y (integral-rev (delay dy) y0 dt))
  (define dy (stream-map f y))
  y)

;(stream-ref 
; (solve-rev (lambda (y) y) 1 0.001) 1000)
;2.716924 - works in Racket again!
...
(define (solve-2d a b y0 dy0 dt)
  (define y  (integral-rev (delay dy) y0 dt))
  (define dy (integral-rev (delay ddy) dy0 dt))
...
(define (solve-2d-gen f y0 dy0 dt)
  (define y  (integral-rev (delay dy) y0 dt))
  (define dy (integral-rev (delay ddy) dy0 dt))
  (define ddy (stream-map f y dy)) 
  y)

;with some help from wizard solutions
...
(define (RLC R L C dt)
  (lambda (vC0 iL0)
    ...
    (define vC  (integral-rev (delay dvC) vC0 dt))
    (define iL  (integral-rev (delay diL) iL0 dt))
