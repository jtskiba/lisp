...
(define (scanning var vars vals env lambda-null lambda-else)
  (cond ((null? vars)
         lambda-null)
        ((eq? var (car vars))
         lambda-else)
    ...
    (if (eq? env the-empty-environment)
        ...
        (let ((frame (first-frame env)))
          (scanning
           ...
           (frame-values frame)
           env
           ((lambda (e) (env-loop (enclosing-environment e))) env)
           ((lambda (vls vl) (set-car! vls vl)) (frame-values frame) val)))))
  (env-loop env))
...
;     (frame-values frame)
;     env
;     (lambda (vr vl fr) (add-binding-to-frame! vr vl fr))
;     (lambda (vls vl) (set-car! vls vl)))))
