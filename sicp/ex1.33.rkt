...
(define (relatively-prime b a)
  (= (gcd a b) 1))

(define (filtered-accumulate filter combiner base term a next b)
  (if (> a b)
      base
      (combiner (term (if (filter a (+ b 1))
                          a
                          base))
         (filtered-accumulate filter combiner base term (next a) next b))))

(accumulate * 1 square 1 inc 4)
(filtered-accumulate prime? * 1 square 1 inc 4)
(filtered-accumulate relatively-prime * 1 identity 1 inc 4)

;(relatively-prime 3 9)