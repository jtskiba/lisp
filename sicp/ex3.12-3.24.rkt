...
;first 2D re-write using lists
(define (make-2D-table)
  (let ((local-table (mlist '*table*)))
    (define (lookup keys)
      (let ((subtable 
             (assoc (mcar keys) (mcdr local-table))))
        (if subtable
            (let ((record 
                   (assoc (mcar (mcdr keys)) 
                          (mcdr subtable))))
              (if record (mcdr record) #f))
            #f)))
        ...
        (if subtable
            (let ((record 
              ...
              (if record
                  ...
                  (set-mcdr! 
                   subtable
                   (mcons (mcons (mcar (mcdr keys)) value)
                         (mcdr subtable)))))
            (set-mcdr! 
             local-table
             (mcons (mlist (mcar keys)
                         (mcons (mcar (mcdr keys)) value))
                   (mcdr local-table)))))
      'ok)
    (define (dispatch m)
      (cond ((eq? m 'lookup-proc) lookup)
 ...
 (define num-key (make-2D-table))
((num-key 'insert-proc!) (mlist 10 1) 33)
((num-key 'insert-proc!) (mlist 10 2) 30)
((num-key 'insert-proc!) (mlist 12 1) 195)
((num-key 'insert-proc!) (mlist 12 3) 210)
((num-key 'lookup-proc) (mlist 12 3))
((num-key 'insert-proc!) (mlist 12 3) 212)
((num-key 'lookup-proc) (mlist 12 3))
...
;        (let ((subtable 
;               (assoc-num (mcar keys) (mcdr table))))
;          (if subtable
;              (let ((record 
;                     (assoc-num (mcar (mcdr keys)) 
;                                (mcdr subtable))))
;                (if record (mcdr record) #f))
;              #f)))
    ...
    (define (lookup keys)
      (define (iter keys table)
        (let ((subtable 
          ...
          (if subtable
              (if (mpair? (mcar (mcdr subtable)))
                  (let ((record 
                         (assoc (mcar (mcdr keys)) 
                                    (mcdr subtable))))
                    (if record (mcdr record) #f))
                  (iter (mcdr keys) subtable)) ; i am not sure if this line works (pick #f otherwise)
                  #f)))
      (iter keys local-table))
;    (define (insert! keys value)
;      (let ((subtable 
;             (assoc-num (mcar keys) (mcdr local-table))))
;        (if subtable
;            (let ((record 
;                   (assoc-num (mcar (mcdr keys)) 
;                          (mcdr subtable))))
              ...
              (if (mpair? (mcar (mcdr subtable)))
                  (let ((record 
                    ...
                    (if record
                        ...
                        (set-mcdr! 
                         subtable
                         (mcons (mcons (mcar (mcdr (mcdr keys))) value)
                                (mcdr subtable)))))
                  (iter (mcdr keys) subtable))                
              (set-mcdr! 
               table
               (mcons (mlist (mcar keys)
                             (mcons (mcar (mcdr keys))
                                    (mcons (mcar (mcdr (mcdr keys)))
                                           value)))
                      (mcdr table)))))
        'ok)
      (iter keys local-table))
...
;((nd-key 'insert-proc!) (mlist 'jarek 'car 'age) 15)
;((nd-key 'insert-proc!) (mlist 'jarek 'house 'sqm) 100)
;((nd-key 'insert-proc!) (mlist 'jarek 'house 'bedrooms) 3)
;((nd-key 'insert-proc!) (mlist 'marta 'car 'make) 'polonez)
;((nd-key 'insert-proc!) (mlist 'marta 'car 'age) 10)
;((nd-key 'insert-proc!) (mlist 'marta 'house 'sqm) 90)
;((nd-key 'insert-proc!) (mlist 'marta 'house 'bedrooms) 4)
