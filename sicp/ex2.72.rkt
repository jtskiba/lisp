...
;ex2.72
; my answer was n^2 as for each of n letters in a message  we need to find a code which can be either at first left branch 1
; or it will be at far end of the branch n-1 bit long, so on average for each letter we need to search through ~n/2 branches
; hence on average we get n* (n/2) so order of growth ~n^2 (but quickest is n, and least good is n^2 -n, so n^2 (as -n can be ignored))
